...
function cascade_value_update(obj) {
...
$(document).on("change", "select.cascade-list", function(e, opt) {
    ...
    if (v) {
        ...
        $.get(href, function(data, status) {
            ...
            if (obj.attr("multiple") && obj.find("option").length < 50) {
                if (obj.parent().find(".help-block").length == 0) {
                    obj.parent().append('<span class="help-block">Ctrl + shift + click to select all</span>');
                }
            } else {
                obj.parent().find(".help-block").last().remove();
            }

            if (auto_load == "1") {
                cascade_value_update(obj);
            }

            if (obj.hasClass("mt-multiselect")) {
